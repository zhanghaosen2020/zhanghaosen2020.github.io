...
void USART1_Init(void)
{
    ...
    //4. 使能中断
    USART1->CR1 |= USART_CR1_RXNEIE;//用来判断一个字符的接收完成
    USART1->CR1 |= USART_CR1_IDLEIE;//用来判断整个字符串的接受完成
...
void USART1_IRQHandler(void)
{
    ...
    {
        buffer[size++] = USART1->DR; // 读数据寄存器
    }
    else if (USART1->SR & USART_SR_IDLE)  // 空闲中断（表示串口帧结束）
    {
        volatile uint32_t temp;
        temp = USART1->SR;  // 先读 SR
        temp = USART1->DR;  // 再读 DR 来清除 IDLE 标志位
        isOver = 1;         // 标志位置 1，表示一帧接收完成
    }
}
